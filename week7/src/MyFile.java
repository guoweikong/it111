import java.io.File;

public class MyFile {

    public static void main (String[] args) {
        // we are going to create a file object

//        File dir = new File("NewFolder");
        File dir = new File("/Users/mingqiangniu/IdeaProjects/week7/NewFolder");
        // inside(), we need call out folder

        // create an if else statement -  if the directory exists, yippy skippy
        // els, not a good thing

        if(dir.exists()){
            System.out.println("Yippy Skippy, we hava a directory");
        } else {
            System.out.println("We are lost in space, don't know where that directory is?");
        }
    }
}
